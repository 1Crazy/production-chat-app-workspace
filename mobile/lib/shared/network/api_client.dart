...
String formatDisplayError(Object error, {String fallback = '操作失败，请稍后再试'}) {
  ...
  if (error is SocketException) {
    return '无法连接服务器，请确认服务已启动并且网络可用';
  }

  if (error is http.ClientException) {
    final networkMessage = _friendlyInfrastructureMessage(error.toString());
    return networkMessage ?? '网络请求失败，请稍后再试';
  }
  ...
  final networkMessage = _friendlyInfrastructureMessage(raw);
...
String? _friendlyInfrastructureMessage(String raw) {
